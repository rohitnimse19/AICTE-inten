...
const getUserbyUsername = async (req, res) => {
  try {
    ...
    const foundUser = await User.find({ username: username }).select(
      "-password"
    );
...
const getAllRecurringTransactions = async (req, res) => {
  try {
    ...
    if (recurringTransactions.length === 0) {
      return res
        .status(404)
        .json({ message: "no Recurring Transaction found" });
    }
...
const deleteRecurringTransactionById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Successfully delted.", recurringTransaction });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal server error" });
  }
