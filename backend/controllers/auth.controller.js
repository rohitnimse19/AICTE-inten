...
const signup = async (req, res) => {
  try {
    const { username, email, password, fname, lname, age } = req.body;
...
const login = async (req, res) => {
  try {
    ...
    if (!accessSecretKey)
      return res.status(403).json({ message: "Missing JWT secret key" });
    ...
    if (!refreshSecretKey)
      return res
        .status(403)
        .json({ message: "Missing JWT refresh secret key" });
  ...
  try {
    ...
    if (!foundUser) {
      jwt.verify(refreshToken, refreshSecretKey, async (error, decoded) => {
        ...
        console.log(
          `Hacked user ${hackedUser.username} logged out of all devices`
        );
      });
      return res.status(403).json({ message: "Forbidden" });
    }
